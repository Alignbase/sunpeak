...
<Note>
  This server example uses the MCP SDK 1.x package used by sunpeak 0.20.x. Standalone projects on
  ext-apps 2.0 must use the split MCP SDK packages and `z.object()` schemas. See the [SDK 2.0
  migration guide](/mcp-apps/sdk-v2-migration).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Tool and Resource Contract" icon="page" href="/mcp-apps/server/tool-resource-contract">
    Check the fields that connect MCP tools, UI resources, and tool results.
  </Card>
  <Card title="Requirements Checklist" icon="page" href="/mcp-apps/server/requirements">
    Verify portability, fallback behavior, CSP, annotations, and host testing.
  </Card>
  <Card title="Tool Annotations" icon="page" href="/mcp-apps/server/tool-annotations">
