---
title: 'MCP Apps CSP & CORS'
sidebarTitle: 'CSP & CORS'
description: 'Configure Content Security Policy and CORS for MCP Apps that load APIs, assets, media, or nested frames from sandboxed iframes.'
keywords:
  [
    'MCP Apps CSP',
    'MCP Apps CORS',
...
Read the [MCP Apps Security Model](/mcp-apps/security) first if you need the full trust boundary, tool authorization, message validation, and result-data rules.
...
<Warning>
  CSP is restrictive by default. If you don't declare domains, no external connections are allowed.
  This prevents data exfiltration to undeclared servers.
</Warning>
...
<Info>
  The `domain` format is host-specific. Check each host's documentation for its supported format.
  The Claude pattern above uses SHA-256 hashing of the MCP server URL.
</Info>
...
<Card
  horizontal
  title="McpUiResourceCsp"
  icon="code"
  href="/mcp-apps/types/core-types#mcpuiresourcecsp"
>
  TypeScript type definition for CSP fields.
</Card>

<Tip>
  In the [sunpeak framework](/quickstart), set CSP in your resource file's `_meta.ui.csp` field. See
  the [resource file reference](/app-framework/resources/albums) for an example.
</Tip>
