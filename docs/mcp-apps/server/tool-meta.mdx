...
MCP App tools use `_meta.ui` to link a tool to an HTML View, control who can access it, or both. This metadata is set in the tool config passed to [`registerAppTool`](/mcp-apps/server/register-app-tool).
...
For UI-launching tools, this must match a URI registered via [`registerAppResource`](/mcp-apps/server/register-app-resource). When the host invokes the tool, it calls [`resources/read`](/mcp-apps/mcp/resources#reading-resourcesread) with this URI to fetch the HTML content, then renders it in a sandboxed iframe.
...
| `"app"`   | Tool is callable by the View (your UI) via [`app.callServerTool()`](/mcp-apps/app/requests/call-server-tool) on the same server connection |
...
<Tip>
  The [sunpeak framework](/quickstart) provides [`useAppTools`](/app-framework/hooks/use-app-tools)
  for calling app-visible tools from React components, and the [Tool File
  Reference](/app-framework/tools/tool-file) for defining tool metadata.
</Tip>

For a complete field-by-field checklist across tools, resources, resource metadata, and tool results, see the [Tool and Resource Contract](/mcp-apps/server/tool-resource-contract).
