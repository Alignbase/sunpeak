...
keywords:
  ['MCP Apps', 'ChatGPT Apps', 'register tool', 'MCP server', 'tool visibility', 'input schema']
---
...
Use a runtime schema that supports Standard Schema validation and JSON Schema export. `z.object()`
with Zod 4.2 or later meets both requirements. SDK 2.0 still accepts raw Zod shapes, but that
overload is deprecated. See [Tool Schemas](/mcp-apps/server/tool-schemas) for validation behavior and
the [SDK 2.0 migration guide](/mcp-apps/sdk-v2-migration#wrap-tool-schemas-with-zobject).
...
<ResponseField name="config" type="McpUiAppToolConfig" required>
  ...
  <ResponseField name="inputSchema" type="ZodRawShape | StandardSchemaWithJSON">
    Runtime schema for tool arguments. The recommended form implements Standard Schema validation
    and `~standard.jsonSchema`; a raw Zod shape is deprecated. The SDK exports JSON Schema for
    `tools/list` and parses arguments before the callback runs.
  </ResponseField>
  <ResponseField name="outputSchema" type="ZodRawShape | StandardSchemaWithJSON">
    Runtime schema for successful `structuredContent`. The SDK exports JSON Schema for `tools/list`
    and validates successful results after the callback returns.
  </ResponseField>
...
See **[Tool Schemas](/mcp-apps/server/tool-schemas)** for `inputSchema`, `outputSchema`, runtime
validation, Standard Schema requirements, and common errors.
