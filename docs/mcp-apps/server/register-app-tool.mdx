...
<Note>
  This reference uses the raw Zod shape accepted by SDK 1.7.5. In SDK 2.0, wrap input and output
  shapes with `z.object()`. The raw shape overload is deprecated. See the [SDK 2.0 migration
  guide](/mcp-apps/sdk-v2-migration#wrap-tool-schemas-with-zobject).
</Note>
