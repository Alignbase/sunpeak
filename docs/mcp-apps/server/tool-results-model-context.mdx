...
| `structuredContent`  | Tool handler                           | Model, View, and compatible clients  | JSON data the model may read and the UI renders, matching `outputSchema`.    |
...
Core MCP `2026-07-28` allows any JSON value in a tool result's `structuredContent`; the prior `2025-11-25` revision requires an object. Most MCP Apps should keep an object root for cross-version portability. See [MCP structuredContent, content, and _meta](/mcp-apps/server/content-structuredcontent-meta#json-values-and-protocol-versions) for the compatibility table.
...
<Note>
  Tool result `structuredContent` and `updateModelContext().structuredContent` have different root
  types. Current core tool results may use any JSON value, but MCP Apps model context remains a
  `Record<string, unknown>`. Wrap arrays, scalars, and null in a named object field before calling
  `updateModelContext()`.
</Note>
