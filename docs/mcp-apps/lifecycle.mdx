...
- **`ui/notifications/tool-result`** -- Tool execution result from the server, including both `content` (readable text) and `structuredContent` (typed data the model may read and the UI can render)
