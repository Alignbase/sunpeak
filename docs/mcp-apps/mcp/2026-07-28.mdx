...
| Layer | Current standard | Current sunpeak |
| --- | --- | --- |
| Host ↔ MCP server | Core MCP `2026-07-28` | Core MCP `2025-11-25` through the SDK 2.0 legacy compatibility transport |
| View ↔ Host | MCP Apps `2026-01-26` | MCP Apps `2026-01-26` through `@modelcontextprotocol/ext-apps` v2.0 |

<Warning>
  sunpeak's `--stateless` option creates a fresh MCP server for each HTTP request, but it does
...
The MCP TypeScript SDK implements the 2026 revision in its v2 API, and `@modelcontextprotocol/ext-apps@2.0.0` uses those split packages. Current sunpeak releases use the same 2.0 package line for their Views, inspector client, and server helpers. The framework's public HTTP wrappers still use the SDK's legacy Streamable HTTP transport, so a separate server-routing change is required before sunpeak can advertise the modern wire protocol. See the [MCP Apps SDK 2.0 Migration Guide](/mcp-apps/sdk-v2-migration) for package and TypeScript API changes.
...
### OAuth authorization is stricter

Clients validate the authorization response's `iss` parameter before redeeming a code and keep
persisted credentials bound to the authorization server issuer. Dynamic Client Registration now
requires the client to send an appropriate `application_type`; it is deprecated in favor of Client
ID Metadata Documents and kept for backwards compatibility.

These rules apply to the Host-to-server connection. The View still calls tools through the Host and
should never receive the MCP access token. See [MCP OAuth Authorization for MCP
Apps](/mcp-apps/mcp/authorization) for the complete flow and server checklist.
...
- `structuredContent` may be any JSON value when its `outputSchema` permits it. See [MCP structuredContent, content, and _meta](/mcp-apps/server/content-structuredcontent-meta#json-values-and-protocol-versions) for legacy and MCP Apps compatibility.
...
| MCP server | sunpeak inspector and test client | 2026-capable Host |
| --- | --- | --- |
| Legacy `2025-11-25` | Supported through automatic fallback | Supported when the Host can fall back to the legacy era |
| Dual-era | Supported through the 2026 path or legacy fallback | Supported through the 2026 path or legacy fallback |
| Modern-only `2026-07-28` | Supported | Supported |

The sunpeak inspector and test client now use the SDK 2.0 client's automatic version negotiation. A modern-only server should still return a clear error when a separate legacy client sends `initialize`, because that client has no way to negotiate forward.

## Prepare a sunpeak App for the Modern Wire Protocol
...
4. Keep OAuth client state and credentials bound to the authorization server issuer.
5. Keep tool handlers independent of in-memory connection state so any request can run on any instance.
6. Test a dual-era server with both 2025 and 2026 clients before dropping legacy support.
7. Upgrade the MCP SDK and MCP Apps SDK only when their supported version ranges overlap.
...
<CardGroup cols={2}>
  ...
  <Card title="MCP Apps SDK 2.0 Migration" icon="page" href="/mcp-apps/sdk-v2-migration">
    Package, schema, handler, and error changes in ext-apps 2.0.
  </Card>
</CardGroup>
