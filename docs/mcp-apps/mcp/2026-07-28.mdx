...
The MCP TypeScript SDK implements the 2026 revision in its v2 API. `@modelcontextprotocol/ext-apps@2.0.0` uses those split v2 packages, but sunpeak 0.20.x still uses ext-apps 1.7.5 and the monolithic v1 MCP SDK. Do not upgrade either package independently in a sunpeak project. See the [MCP Apps SDK 2.0 Migration Guide](/mcp-apps/sdk-v2-migration) for the standalone SDK boundary.
