...
sidebarTitle: "FAQ"
description: "Answers to common questions about MCP Apps: SDK versions, supported hosts, UI delivery, iframe security, display modes, theming, and local testing."
keywords: ["MCP Apps", "MCP Apps FAQ", "ChatGPT Apps", "ChatGPT plugins", "Claude Apps", "MCP app framework", "ui:// resource", "MCP Apps SDK", "MCP Apps SDK 2.0", "how to build an MCP App", "MCP App iframe", "MCP App display modes"]
---
...
Keep the tool name, input schema, annotations, and readable `content`. Add a `ui://` HTML resource, return model-safe `structuredContent` for the View, declare an `outputSchema`, and link the tool with `_meta.ui.resourceUri`. Keep a text-only registration branch for hosts that do not advertise MCP Apps support. See [Add a UI to an MCP Tool](/mcp-apps/server/add-ui-to-tool).
</Accordion>
...
<Accordion title="Should I use MCP Apps SDK 1.x or 2.0?">
Use the SDK version required by your framework. sunpeak 0.20.x uses `@modelcontextprotocol/ext-apps` 1.7.5 and the MCP TypeScript SDK 1.30, so do not replace them independently. Standalone apps can use ext-apps 2.0 with the split MCP SDK packages and Zod 4.2 or newer. The View-to-Host wire protocol remains `2026-01-26`, so 1.x and 2.x Views and Hosts can communicate. See the [MCP Apps SDK 2.0 Migration Guide](/mcp-apps/sdk-v2-migration) for install commands and TypeScript changes.
</Accordion>
