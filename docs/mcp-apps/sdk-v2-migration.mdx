...
<Warning>
  sunpeak 0.20.x uses `@modelcontextprotocol/ext-apps` 1.7.5 and
  `@modelcontextprotocol/sdk` 1.30. Do not replace either dependency with 2.0 in a sunpeak
  project. Use sunpeak's exports and wait for a sunpeak release that adopts the split MCP SDK
  packages.
</Warning>

This guide is for standalone MCP Apps and Hosts that import `@modelcontextprotocol/ext-apps` directly, or for teams preparing a future sunpeak upgrade.
...
Version 2 requires Node.js 20 or newer. `@modelcontextprotocol/core` is also a required peer, but package managers install it through `@modelcontextprotocol/client`, so applications do not normally list it directly.
...
<CardGroup cols={2}>
  ...
  <Card title="Protocol Reference" icon="page" href="/mcp-apps/types/protocol-reference">
    The 1.7.5 types and schemas currently used by sunpeak 0.20.x.
  </Card>
</CardGroup>
