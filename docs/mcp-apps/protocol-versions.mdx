...
| -------------------- | ----------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------- |
| Core MCP protocol    | Latest: `2026-07-28`; sunpeak HTTP wrappers: `2025-11-25` | Host ↔ MCP server requests                         | Tools, resources, transport, and core MCP capabilities              |
| MCP Apps protocol    | `2026-01-26`                                          | View ↔ Host `ui/initialize`                           | View lifecycle, host context, app capabilities, and `ui/*` messages |
| MCP Apps SDK package | `2.0.0`                                               | `package.json`                                        | The implementation, peer dependencies, and TypeScript APIs          |
...
## SDK 2.0 Changes APIs, Not the View Protocol

`@modelcontextprotocol/ext-apps` 2.0 uses the split MCP TypeScript SDK 2.x packages. It changes dependencies, Zod requirements, handler context fields, custom handler registration, and error classes. It does not change the MCP Apps `2026-01-26` messages exchanged between the View and Host.

A 2.x View can run in a 1.x Host, and a 2.x Host can render a 1.x View. This wire compatibility does not make their TypeScript classes interchangeable, and it does not make the core MCP 1.x and 2.x server APIs interchangeable.

Current sunpeak releases use ext-apps 2.0 with the split client, core, server, and Node SDK packages. sunpeak's existing HTTP server wrappers continue to use the SDK's handshake-based compatibility transport, so adopting the 2.0 TypeScript APIs does not by itself switch those endpoints to the core `2026-07-28` wire protocol. Standalone projects can use the [MCP Apps SDK 2.0 Migration Guide](/mcp-apps/sdk-v2-migration) to move to the split packages.
...
sequenceDiagram
    ...
    alt Core MCP 2025-11-25, used by sunpeak HTTP wrappers
...
Core MCP `2026-07-28` removes this handshake and carries the version, identity, and capabilities on each request. sunpeak uses the SDK 2.0 package APIs, while its framework HTTP wrappers still expose the legacy compatibility transport. See [MCP 2026-07-28 for MCP Apps](/mcp-apps/mcp/2026-07-28) for the compatibility boundary and migration checklist.
...
| Protocol validation fails after an upgrade     | For 1.x, check `@modelcontextprotocol/sdk`. For 2.x, check the split client, server, core, and Zod versions. Then compare the payload with the exported schemas. |
...
<CardGroup cols={2}>
  ...
  <Card
    title="Core MCP 2026-07-28 Versioning"
    icon="link"
  ...
  <Card title="MCP 2026-07-28 for MCP Apps" icon="page" href="/mcp-apps/mcp/2026-07-28">
  ...
  <Card title="MCP Apps SDK 2.0 Migration" icon="page" href="/mcp-apps/sdk-v2-migration">
    Split package installs, TypeScript API changes, and cross-version compatibility.
  </Card>
