...
This page ties together the four pieces that make app-side tools work: the [`tools` capability](#1-declare-the-tools-capability), [`onlisttools`](#2-declare-which-tools-the-app-exposes), [`oncalltool`](#3-handle-tool-calls), and list-changed notifications for dynamic tool lists.
...
Add, update, enable, disable, or remove tools at runtime. Each call returns a handle with `enable()`, `disable()`, `remove()`, and `update()`. The SDK builds the tool list, dispatches calls, and sends list-changed notifications automatically when the capability is enabled.
...
When you call `app.registerTool()` before `connect()`, the SDK declares `{ tools: { listChanged: true } }` automatically if you did not provide a `tools` capability. Manual `onlisttools` and `oncalltool` handlers still require you to declare the capability yourself.
...
The schema must support both Standard Schema validation and JSON Schema export. The SDK validates
arguments before the callback, validates successful `structuredContent` when you add
`outputSchema`, and exports both schemas to the host. See [Tool
Schemas](/mcp-apps/server/tool-schemas#app-side-tool-schemas) for a complete input and output
example.
...
| Method | Effect |
|--------|--------|
| `update(config)` | Update the title, description, schemas, annotations, metadata, or enabled state |
...
The dynamic registry sends `notifications/tools/list_changed` automatically after `registerTool()`, `update()`, `enable()`, `disable()`, or `remove()` when the View has initialized with `listChanged: true`.

```ts
const saveHandle = app.registerTool("save", { description: "Save changes" }, saveHandler);

saveHandle.disable(); // SDK notifies the host
saveHandle.enable(); // SDK notifies the host again
```

Use [`sendToolListChanged()`](/mcp-apps/app/requests#sendtoollistchanged) when you manage a custom `onlisttools` result outside the registry:

```ts
let exposeSelectionTool = false;

editor.on("dirty", async () => {
  exposeSelectionTool = true;
  await app.sendToolListChanged();
});
```

List-changed notifications work only when the View declared `tools: { listChanged: true }` and the Host supports app-side tools. The Host then re-runs `tools/list` and updates the model's available tools for the next turn.
...
- [Tool Schemas](/mcp-apps/server/tool-schemas) — input, output, and runtime validation
