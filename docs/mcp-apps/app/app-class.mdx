...
The `App` class provides a framework-agnostic way to build interactive MCP Apps that run inside host applications. In SDK 2.0, which current sunpeak releases use, it extends the split client SDK's `Protocol` class and handles the connection lifecycle, [initialization handshake](/mcp-apps/lifecycle#2-initialization), and bidirectional communication with the host. The class provides DOM-style `addEventListener()` and `removeEventListener()` methods alongside the traditional `on*` setter properties.

<Note>
  MCP Apps SDK 2.0 removed the `ProtocolWithEvents` type. Import `App` or `AppBridge` instead; both
  keep the event methods. See the [SDK 2.0 migration guide](/mcp-apps/sdk-v2-migration).
</Note>
...
<ResponseField name="options" type="AppOptions" default="{ autoResize: true }">
  ...
  <ResponseField name="allowUnsafeEval" type="boolean" default="false">
    Allow schema parsing paths that require CSP `unsafe-eval`. Keep this disabled for normal MCP
    App Views.
  </ResponseField>
</ResponseField>
...
Host-bound methods such as `callServerTool()`, `sendMessage()`, and `updateModelContext()` must run after `connect()` completes. Calling them earlier can race the `ui/initialize` handshake and leave a strict Host waiting for a View that never becomes ready.
...
// Host-bound methods are safe after connect() resolves.
await app.updateModelContext({
  content: [{ type: "text", text: "The View is ready." }],
});
```

### CSP-safe schema parsing

MCP App Views run under a restrictive Content Security Policy. By default, the SDK configures Zod's parser to avoid generated functions that require `unsafe-eval`.
...
For the difference between the core MCP handshake, the View handshake, the SDK package version, and `appInfo.version`, see [Protocol Versions and Compatibility](/mcp-apps/protocol-versions).
