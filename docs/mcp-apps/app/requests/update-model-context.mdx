...
description: "Update the host's model context with MCP App state. Send content blocks or an object-shaped structuredContent value for future model turns."
keywords: ["MCP Apps", "ChatGPT Apps", "updateModelContext", "model context", "structuredContent object", "app state", "app requests"]
---
...
<Note>
  `updateModelContext().structuredContent` must be an object. This differs from a core MCP
  `CallToolResult`, whose `structuredContent` may be any JSON value in protocol `2026-07-28`.
  Wrap an array, scalar, or null in a named object field before sending it as model context.
</Note>
