...
description: "Make requests from an MCP App View to the host: call server tools, request LLM completions via sampling, send messages, update model context, open links, download files, request display modes, and send logs."
keywords: ["MCP Apps", "ChatGPT Apps", "app requests", "server tools", "send message", "model context", "sampling", "createSamplingMessage"]
---
...
Notify the host that a custom app-side tool list has changed. Use this when you implement `onlisttools` yourself and its result changes.

The SDK's `registerTool()` registry and the returned handle's `update()`, `enable()`, `disable()`, and `remove()` methods send this notification automatically when `listChanged` is enabled. Do not send a duplicate notification after those calls.
