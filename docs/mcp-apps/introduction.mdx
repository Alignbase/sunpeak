...
<Note>
  This API reference matches MCP Apps SDK v2.0.0, which current sunpeak releases use. The iframe
  wire protocol is compatible with 1.x Views and Hosts, but dependencies and some TypeScript APIs
  changed. See the [SDK 2.0 migration guide](/mcp-apps/sdk-v2-migration) when upgrading an older
  standalone app or sunpeak integration.
</Note>
...
If you maintain a standalone MCP App on `@modelcontextprotocol/ext-apps` 1.x, use the [MCP Apps SDK 2.0 Migration Guide](/mcp-apps/sdk-v2-migration) for package, schema, handler, and error changes. Current sunpeak releases already manage the split MCP 2.x packages for framework projects.
