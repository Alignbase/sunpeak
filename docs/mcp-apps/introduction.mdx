...
<Note>
  This API reference matches MCP Apps SDK v1.7.5, which sunpeak 0.20.x uses. The upstream
  `@modelcontextprotocol/ext-apps` latest release is 2.0.0. Its iframe wire protocol is compatible,
  but its dependencies and some TypeScript APIs changed. See the [SDK 2.0 migration
  guide](/mcp-apps/sdk-v2-migration) before upgrading a standalone app.
</Note>
...
All Views run in separate-origin sandboxed iframes with no access to the Host's DOM, cookies, or storage. The Host validates and mediates JSON-RPC messages sent through `postMessage`.
...
Start with the [MCP Apps Security Model](/mcp-apps/security) for the trust boundaries, tool authorization rules, result-data audiences, and pre-ship checks. Use [CSP and CORS](/mcp-apps/server/csp-cors) when configuring external origins.
...
If you are wiring the server side from scratch, start with [Build an MCP App Server](/mcp-apps/server/build-server). If you already have a normal MCP tool and want to add an interactive View, use [Add a UI to an MCP Tool](/mcp-apps/server/add-ui-to-tool). Before testing in a host, use the [MCP App Requirements Checklist](/mcp-apps/server/requirements) to verify portability, fallback behavior, CSP, annotations, and result data. For a field-by-field check, use the [Tool and Resource Contract](/mcp-apps/server/tool-resource-contract). For result payloads, use [Tool Results and Model Context](/mcp-apps/server/tool-results-model-context) to decide what belongs in `content`, `structuredContent`, `_meta`, and `updateModelContext`.
...
If you maintain a standalone MCP App on `@modelcontextprotocol/ext-apps` 1.x, use the [MCP Apps SDK 2.0 Migration Guide](/mcp-apps/sdk-v2-migration) for package, schema, handler, and error changes. sunpeak 0.20.x projects should stay on the framework-managed 1.7.5 SDK until sunpeak adopts the split MCP 2.x packages.
