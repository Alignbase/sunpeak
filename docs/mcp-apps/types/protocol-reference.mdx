...
<Note>
  This reference covers MCP Apps SDK v2.0.0, which current sunpeak releases use. Version 2.0 keeps
  the same View protocol, uses split peer packages, deprecates the union types below, and broadens
  some core schemas. See the [SDK 2.0 migration guide](/mcp-apps/sdk-v2-migration).
</Note>
...
The host proxies select MCP requests between the View and the originating MCP server (or, for sampling, between the View and the host's LLM). These reuse standard MCP request/result types from `@modelcontextprotocol/client` in View and Host code, or `@modelcontextprotocol/server` in server code.

| Type | Method | Direction | Description |
|------|--------|-----------|-------------|
| `CallToolRequest` | `tools/call` | View → Server | Call a server tool. Used by [`callServerTool`](/mcp-apps/app/requests/call-server-tool). |
| `CallToolResult` | — | Server → View | Tool execution result. |
| `ReadResourceRequest` | `resources/read` | View → Server | Read a server resource. Used by [`readServerResource`](/mcp-apps/app/requests/read-server-resource). |
| `ReadResourceResult` | — | Server → View | Resource contents. |
| `ListResourcesRequest` | `resources/list` | View → Server | List server resources. Used by [`listServerResources`](/mcp-apps/app/requests/list-server-resources). |
| `ListResourcesResult` | — | Server → View | Resource list with optional cursor. |
| `CreateMessageRequest` | `sampling/createMessage` | View → Host | LLM completion. Used by [`createSamplingMessage`](/mcp-apps/app/requests/create-sampling-message). |
...
| `McpUiSandboxResourceReadyNotification` | `ui/notifications/sandbox-resource-ready` | Resource HTML is ready for a sandbox proxy |
...
| `McpUiSandboxProxyReadyNotification` | `ui/notifications/sandbox-proxy-ready` | Sandbox proxy is ready to receive resource HTML |
| `McpUiRequestTeardownNotification` | `ui/notifications/request-teardown` | App requests host to tear it down. Sent by [`requestTeardown`](/mcp-apps/app/requests#requestteardown). |
| `LoggingMessageNotification` | `notifications/message` | Diagnostic log message. Sent by [`sendLog`](/mcp-apps/app/requests/send-log). |
| `ToolListChangedNotification` | `notifications/tools/list_changed` | App-side tool list changed. The dynamic registry sends it automatically; custom `onlisttools` implementations can use [`sendToolListChanged`](/mcp-apps/app/requests#sendtoollistchanged). |
...
| `SANDBOX_PROXY_READY_METHOD` | `"ui/notifications/sandbox-proxy-ready"` |
| `SANDBOX_RESOURCE_READY_METHOD` | `"ui/notifications/sandbox-resource-ready"` |
...
| `McpUiSandboxProxyReadyNotificationSchema` | `McpUiSandboxProxyReadyNotification` — sandbox proxy ready |
| `McpUiSandboxResourceReadyNotificationSchema` | `McpUiSandboxResourceReadyNotification` — resource HTML ready |
