...
| Field | Description |
|-------|-------------|
| `experimental` | Experimental feature configuration keyed by feature identifier |
| `openLinks` | Host supports [`openLink()`](/mcp-apps/app/requests/open-link) |
| `downloadFile` | Host supports [`downloadFile()`](/mcp-apps/app/requests/download-file) |
| `serverTools` | Host can proxy [`callServerTool()`](/mcp-apps/app/requests/call-server-tool) to the MCP server |
| `serverResources` | Host can proxy [`readServerResource()`](/mcp-apps/app/requests/read-server-resource) to the MCP server |
| `logging` | Host accepts [`sendLog()`](/mcp-apps/app/requests/send-log) messages |
| `sandbox` | Permissions and CSP applied by the host |
| `updateModelContext` | Host accepts [`updateModelContext()`](/mcp-apps/app/requests/update-model-context) |
| `message` | Host accepts [`sendMessage()`](/mcp-apps/app/requests/send-message) |
| `sampling` | Host supports [`createSamplingMessage()`](/mcp-apps/app/requests/create-sampling-message) for LLM completions. `sampling.tools` indicates tool-augmented sampling support. |
...
The [sunpeak framework](/quickstart) also exports a short alias: `Theme` (equivalent to `McpUiTheme`).
...
The [sunpeak framework](/quickstart) also exports a short alias: `DisplayMode` (equivalent to `McpUiDisplayMode`).
...
<Tip>
The [sunpeak framework](/quickstart) provides convenience hooks for accessing these types reactively: [`useHostContext`](/app-framework/hooks/use-host-context), [`useHostInfo`](/app-framework/hooks/use-host-info), and [`useTheme`](/app-framework/hooks/use-theme).
</Tip>
