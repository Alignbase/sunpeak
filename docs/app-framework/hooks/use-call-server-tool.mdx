...
<ResponseField name="structuredContent" type="unknown">
  JSON data from the tool response. Version 2 permits objects, arrays, and primitive JSON values when the tool's output schema allows them.
</ResponseField>

<Note>
  sunpeak's handshake-based HTTP server wrappers preserve compatibility with the `2025-11-25`
  protocol, which represents a non-object structured result as `{ result: value }` on that wire.
</Note>
