...
The underlying MCP Apps registry sends `notifications/tools/list_changed` automatically when you register, update, enable, disable, or remove a tool and the View declared `tools.listChanged`.
...
function SelectionTool({ selectedText }: { selectedText: string }) {
  ...
  }, [registerTool, selectedText]);

  return <div>Selection tool registered</div>;
}
```

Do not call [`useSendToolListChanged()`](/app-framework/hooks/use-send-tool-list-changed) after these registry operations. Use that lower-level hook only when you manage a custom `onlisttools` result.
