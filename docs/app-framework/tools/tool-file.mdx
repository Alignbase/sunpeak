...
export default async function (args: Args, extra: ToolHandlerExtra) {
  ...
  // extra has sessionId, mcpReq, and http
...
<ResponseField name="http.authInfo" type="AuthInfo">
...
<ResponseField name="mcpReq.signal" type="AbortSignal">
  Abort signal for cancellation.
</ResponseField>

<ResponseField name="mcpReq.id" type="RequestId">
  JSON-RPC request identifier.
</ResponseField>

<ResponseField name="mcpReq._meta" type="RequestMeta">
  Metadata from the current MCP request.
</ResponseField>
...
A common pattern is pairing a UI tool (for review) with a backend-only tool (for execution). The review UI calls the backend tool via [`useCallServerTool`](/app-framework/hooks/use-call-server-tool) after the user confirms:
...
Tools without a UI still appear in the inspector's tool picker. Selecting one shows `Tool does not render a UI` in the preview, while the sidebar still lets you edit Tool Input, click **Run**, and inspect Tool Result. They are registered in the dev MCP server, so they can be called by UI resources via `useCallServerTool` or by the host directly. In production, they work as standard MCP tools.
...
<Card horizontal title="Server Entry" icon="server" href="/app-framework/tools/server-entry">
