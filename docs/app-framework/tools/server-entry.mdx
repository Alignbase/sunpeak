...
The optional `src/server.ts` file configures authentication, OAuth protected-resource metadata, and server metadata. When present, the framework calls `auth()` on every MCP request and populates `extra.authInfo` in tool handlers.
