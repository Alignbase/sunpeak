...
The optional `src/server.ts` file configures authentication, OAuth protected-resource metadata, and server metadata. When present, the framework calls `auth()` on every MCP request and populates `extra.http.authInfo` in tool handlers.

Under the hood, sunpeak passes the returned `AuthInfo` to the MCP SDK 2.0 transport, which exposes it in the handler's HTTP context.
...
Called on every MCP request. Return `AuthInfo` to authenticate, `null` to reject the request with 401, or `AuthorizationFailure` to return a specific OAuth error and scope challenge. The second argument includes the parsed request body for request-specific scope checks.
...
**Returns** `AuthInfo | AuthorizationFailure | null` (sync or async):
...
Publishes OAuth 2.0 Protected Resource Metadata and adds its URL to bearer challenges. `sunpeak start` loads this export automatically.

```ts
import type { OAuthProtectedResourceConfig } from 'sunpeak/mcp';
...
See [Authorization](/app-framework/guides/authorization) for scope upgrades, token checks, and custom handler setup.
...
<ResponseField name="icons" type="Icon[]">
  64x64 PNG icons (data URIs preferred). Light and dark variants supported via the `theme` field on
  each icon.
</ResponseField>
