...
<ResponseField name="stateless" type="boolean" default="false">
  Enable stateless mode for serverless and horizontally-scaled deployments. When `true`, every request creates a fresh MCP server instance with no session tracking. The host is identified from HTTP headers (`User-Agent`, `x-anthropic-client`, `x-openai-session`) on every request. See [Serverless Deployment](/app-framework/guides/deployment#serverless--edge-deployment) and [Horizontal Scaling](/app-framework/guides/deployment#horizontal-scaling).
</ResponseField>
