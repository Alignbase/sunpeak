...
| `--sse-keep-alive-ms <number>` | | SSE keep-alive interval in milliseconds when `--sse` is enabled (default: 15000, also respects `SUNPEAK_SSE_KEEP_ALIVE_MS`; `0` disables keep-alives) |
| `--stateless` | | Stateless v1 transport mode: no session tracking and a fresh server per request. This does not enable the MCP `2026-07-28` protocol. For serverless and horizontally-scaled deployments. See [Deployment Guide](/app-framework/guides/deployment#serverless--edge-deployment). |
...
# SSE streaming with a 10s keep-alive
sunpeak start --sse --sse-keep-alive-ms 10000
...
The production server uses the MCP Streamable HTTP transport. In the default stateful mode, session IDs are managed via the `mcp-session-id` header. In `--stateless` mode, only `POST /mcp` is available (no SSE streaming or session deletion). When `--sse` is enabled, sunpeak sends periodic SSE keep-alive comments every 15 seconds by default.
